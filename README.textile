...
If you install as a plugin, be sure to install the @ri_cal@ gem by the method of your choice.
...
<pre>
  ...
  @user.event_specifications.create :description => 'acquire cheese balls', :start_time => Date.today.to_time.utc, :repeat => :daily
...
<pre>
  EventSpecification.create :description => 'pick up laundry', 
    :start_time => Time.parse("4:00pm")
</pre>

h3. Every day
<pre>
  EventSpecification.create :description => 'eat breakfast', 
    :start_time => Time.parse("7:30am"), 
    :repeat => :daily
</pre>
...
<pre>
  EventSpecification.create :description => 'pay taxes', 
    :start_time => Time.parse("4/15/2010 5:00pm"), 
    :repeat => :yearly
</pre>
